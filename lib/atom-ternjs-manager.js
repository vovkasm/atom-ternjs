...
class Manager {
  ...
  setActiveServerAndClient(filePath) {
    ...
    let dir = this.getProjectDirFromPath(filePath);
  ...
  checkPaths(paths) {
    ...
    for (let path of paths) {
      ...
      let dir = this.getProjectDirFromPath(path);
  ...
  // TODO: check destroy logic (may be destroy client)
  destroyServer(paths) {
  ...
  isValidEditor(editor) {
    if (!editor || atom.workspace.isTextEditor(editor) || editor.isMini()) {
      return false;
    }

    let scope = editor.getRootScopeDescriptor().getScopesArray()[0];
  ...
  registerEvents() {
    ...
    this.disposables.push(atom.workspace.onDidStopChangingActivePaneItem((item) => {
      ...
      if (this.isValidEditor(item)) {
        this.setActiveServerAndClient(item.getPath());
      } else {
